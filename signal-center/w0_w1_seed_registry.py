...
ROWS = [
    ...
    ("execution_log",    "Execution Log",     "(signals.db)",          "event_driven", "context",   "executed_at",  "db_max:execution_log.executed_at",  1, "EVENT-DRIVEN (dispatcher direct-execute, sparse — 8 rows ever, only fires on a manual dispatcher trigger); fleet executes via `trades` not here. Reclassed weekly→event_driven 2026-07-06 (HM-OPS-SENTINEL P2.5) — weekly cadence still false-RED'd after 17 days idle between manual triggers; DORMANT is the honest state, not RED."),
    ...
    ("kirk_advisory",    "Kirk Advisory",     "(run-heartbeat)",       "daily",    "context",       "mtime",        "file_mtime:/Users/bigmac/autonomous-trader/data/kirk_advisory.heartbeat", 1, "W1 reads the run-HEARTBEAT (file_mtime), NOT kirk_advisory_log.created_at — the log is event-gated (never logs HOLD, 30-min dedup) so it can't signal liveness. Heartbeat stamped each successful run_kirk_advisory_job (HM-KIRK-REHOME 2026-06-01). GREEN while the job runs, RED if it stops (market-aware daily)."),
