...
def get_positions(conn, snapshot_id):
    cur = conn.execute(
        "SELECT symbol, qty, price, market_value, cost_basis, gain_dollar, gain_pct, asset_type, day_change_pct "
...
def main():
    ...
    cash_balance = 0.0
    positions = []
    for symbol, qty, price, mkt_val, cost_basis, gain_d, gain_pct, asset_type, day_chg_pct in rows:
        if symbol == "CASH":
