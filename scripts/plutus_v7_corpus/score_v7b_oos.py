...
GEN = os.path.join(DATA, "plutus_v7", "gen_v7b_oos.jsonl")
RAW = os.path.join(DATA, "plutus_v7", "judge_raw_v7b_oos.jsonl")
CARD = os.path.join(DATA, "plutus_v7", "scorecard_v7b_oos.json")
...
def generate(scen):
    ...
    for s in scen:
        ...
        out.append({"id": s["id"], "category": s["category"], "v7b_text": txt})
        write_jsonl(GEN, out)
...
def judge(scen, gens):
    ...
    for j in JUDGES:
        ...
        for i, s in enumerate(pending, 1):
            sc, _ = score.judge_one(j, {**s, "decisive_risk": "unknown"}, gens[s["id"]]["v7b_text"])
