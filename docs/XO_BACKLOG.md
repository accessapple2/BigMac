...
| HM-S | `agent_state` table does not exist in live DB but CLAUDE.md and HM-A spec treated it as drawdown-halt source of truth | MEDIUM | `agents/post_earnings_drift.py:56` queries `SELECT is_halted FROM agent_state WHERE agent=?` — table is not in `data/trader.db` (verified 2026-05-04 via `.tables agent%`). Either the drawdown-halt feature is silently dead (queries throw, exceptions swallowed), the module isn't loaded on bigmac, or it queries a different DB. Possible architectural debt — drawdown halt may not actually be implemented. Investigate whether code is dead, errors silent, or queries different DB. **Do NOT touch the queries** until verdict is in — out of HM-B scope. |
...
### HM-S — agent_state table ghost (NEW, surfaced during HM-B pre-flight)
- CLAUDE.md and HM-A spec describe `agent_state.is_halted` as the drawdown-halt source of truth. **Table does not exist in `data/trader.db`** — verified via `.tables agent%` (returns: `agent_allocation, agent_id_aliases, agent_memory, agent_ratings`).
- `agents/post_earnings_drift.py:56` queries `SELECT is_halted FROM agent_state WHERE agent=?` against a non-existent table. Either: feature is dead, exceptions are silently swallowed, or queries hit a different DB.
- Likely architectural debt — drawdown halt feature may not actually be implemented despite being repeatedly cited in halt-system documentation.
- **Investigation, no fix.** Out of HM-B scope; surfaced as a blocker concern but verdict was "ai_brain.py + risk_manager.py reads are local variables, only post_earnings_drift.py:56 reads the missing table — non-blocking for HM-B."
- Recommended next-session prompt: trace whether `post_earnings_drift` is loaded on bigmac and whether its exceptions are swallowed.
