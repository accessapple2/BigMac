...
| `calibration_map.py`'s trade_fire→trade_id join is too narrow to ever be useful | Scotty | unslotted | **Found 2026-09-11**, resolving an apparent contradiction between two numbers (dry-dock B9): last night's 169-trade/78.6%-hit-rate finding (`relay_2026-09-10_phase2-bakeoff-and-phase13-spec.md`) and today's "calibration_map sees 3 rows" are BOTH correct — different populations, not a conflict. `load_calibration_data()` requires `decision_audit.event_type='trade_fire'` with `trade_id` populated JOINed to a closed trade with non-NULL `realized_pnl`; verified by hand-reproducing the query: 193 `trade_fire` events join cleanly to a real trade, but only 3 have settled (non-NULL pnl) — the other 190 are still open. Last night's number bypassed this join entirely (`trades.confidence`/`trades.realized_pnl` queried directly, regime-blind) — 474 clean closed trades, 168 in the 0.9-1.0 bucket, 78.6% hit rate, reproduced fresh (168 vs 169 = one trade's drift over a day, expected). **The real finding: the trade_fire→trade_id linkage `calibration_map` depends on is populated for almost no settled trades (3/474 = 0.6%)** — not a bug exactly, but a design choice that makes the map's own stated methodology nearly unusable in practice regardless of how much more data accumulates, unless the linkage itself gets backfilled/populated more broadly. Worth a decision: relax `calibration_map` to use the same direct `trades`-table population last night's finding used (loses per-decision regime/context precision but gains 158x more usable rows), or invest in backfilling `trade_id` on `trade_fire` events. |
