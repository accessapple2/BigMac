...
| com.ollama.serve down on bigmac | Scotty | unslotted | **Found 2026-09-10 building recall-in-prompt (item B.1).** No `ollama` process running on bigmac at all — absent from `ps aux`, `launchctl list`, and `launchctl print system/com.ollama.serve` ("Could not find service"); the plist (`/Library/LaunchDaemons/com.ollama.serve.plist`, modified Aug 29 18:32) exists but isn't loaded. `SETUP_SIMILARITY_ENABLED`/`RECALL_IN_PROMPT_ENABLED` are both `False` today so nothing live depends on bge-m3 embeddings right now, but `scripts/recall_refresh.py`'s corpus-rebuild job would also fail against this host, and any future recall work (in-prompt or the confirmatory signal) is blocked until it's back. `sudo launchctl bootstrap system ...` needs a password this session doesn't have — needs the Captain (or a scoped NOPASSWD sudoers rule for this specific restart command) to bring it back. Not investigated further tonight (why it's down, how long) — kept tight given tonight's scope. |
