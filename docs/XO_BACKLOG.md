...
| Roster reconciliation: Worf + Troi wired to nothing | Scotty | unslotted | Found 2026-09-10 while triaging the McCoy Ollama-timeout cluster (separate item, not folded in here). `qwen3-8b-flash` (Worf) is `halt_mode='active'` and **is** in `config.AI_PLAYERS`, but produced **zero** rows in `decision_audit` and `crew_decisions` today — not generating any audited decision through either pipeline, cause not yet diagnosed. `options-sosnoff` (Troi) is `halt_mode='active'` in `ai_players` but **absent from `config.AI_PLAYERS` entirely** — the list `agent_routing.py::build_provider` actually routes from — so it cannot produce a live decision regardless of halt state or olliemax load. Distinct from `HM-ORPHAN-SEATS` (2026-07-01, this file line ~7499 — `model_id` pointing at a model no longer served on olliemax): this is upstream of that failure mode — a seat can be `active` with a resolvable model and still never fire because it isn't in the routing config at all (Troi), or is configured correctly but silent for an undiagnosed reason (Worf). Needs: (1) audit every `ai_players` row with `halt_mode='active'` against `config.AI_PLAYERS` membership for gaps like Troi's, (2) diagnose why Worf is active+configured but producing nothing, (3) decide per seat — re-wire or formally retire with a documented reason per this file's Archive Convention. |
| Ollama connection-reuse hardening, round 2 (post second-opinion review) | Scotty | after close | Four follow-ups from the 2026-09-10 stale-socket incident, explicitly deferred out of the 13:00 bundle (which only ships the fresh-connection-per-call fix + config/env + startup logging — see `data/reports/relay/relay_2026-09-10_polygon-limiter-and-ollie-stale-socket.md`). **(1) Streaming with an idle-timeout** instead of the current wall-clock timeout on `/api/generate` — a wall-clock budget kills a call that's genuinely still producing tokens just slowly (e.g. a cold model load), where an idle-timeout (no bytes received for N seconds) would only kill calls that are actually stuck. Needs `stream=True` + reading the NDJSON response incrementally in `engine/providers/ollama_provider.py::_do_request()`, which is a bigger structural change than this incident's fix. **(2) CORRECTED 2026-09-10 post-incident — a cheap health gate before each scan cycle, but NOT `/api/ps`/`/api/tags`.** Original framing (a metadata-listing call) is confirmed insufficient by the actual incident: both `/api/ps` and `/api/tags` stayed green (200 OK, correct model listed) for the full 09:35→11:58 duration while `qwen3:8b`/`plutus-v1` generate calls hung completely — a listing/registry endpoint doesn't exercise the generate serving path at all, so it can't detect this failure class. Needs a real (tiny, cheap) generate probe per model instead — a one-token completion, not a metadata call — before committing a full scan cycle to it. **(3) An integration test that hangs a stub server** (accepts the TCP connection, never writes a response) and asserts the client-side pool doesn't hand back the dead/hung socket for a subsequent call — regression coverage for the actual failure class, not just the cancel-and-log behavior `tests/test_ollama_cancel_on_timeout.py` already covers. **(4) Highest value: a reusable dead-man's-switch monitor class** for `scripts/hm_ops_sentinel.py` — alert when expected work does NOT happen, not just when something errors loudly. Four of this week's real findings were silence, not errors: McCoy's decision rate silently dropping (this incident), a seat producing zero decisions despite being active+configured (Worf, row above), the general shape of "a job that should run periodically just... doesn't, with no exception anywhere to catch," **and (added post-incident 2026-09-10) a single model hanging while the host and every other model answer fine** — the actual live incident (09:35→11:58, `systemctl restart ollama` on olliemax cleared a stuck runner slot; 70 `[OLLAMA-CANCEL]` events, 12,563.52s / 3.49 cumulative hours — see relay doc). A sentinel-level periodic per-model generate probe (same "real call, not a listing endpoint" requirement as item 2 above, but as a background alert rather than a client-side pre-flight gate) is this pattern's concrete next instance. A `check_ollama_decision_health()` design (decision-rate-drop half) was already drafted this session — proposed, not applied, in the same relay doc above — and is a first instance of this pattern; generalizing it into a reusable class (configurable: what table/column or probe, what "was active"/"healthy" floor, what window) rather than a one-off check is the point of this item. |
| bk_orb direct-path pagination re-scope | Scotty | unslotted (a) buildable next session; (b) before any `enforce` flip | **Re-scoped 2026-09-10 PM** against the now-live 100/min cap — see addendum in `relay_2026-09-04_bk-orb-pagination-scope.md`. (a) the 500-bar pagination gap is unaffected by cap size, still a hard prerequisite, safe to build now (`POLYGON_LIMITER_MODE` is still `shadow` — zero live risk either way). (b) budget-starvation risk is 25x de-risked by the cap raise (was ~6 reserved tokens/3min vs a 150-token need, now ~150 vs 150, shared across 7 live callers) but not verified safe under real contention; the staleness-TTL mismatch (30s global bar vs bk_orb's mostly-static 40-day data) is architecturally unchanged. Verdict: ship (a) whenever there's a session for it; do not flip the limiter to `enforce` with bk_orb wired in until (b)'s per-caller TTL override also ships. |
...
| signals.db retention | Scotty | after close, execution order: Phase 1.1 → **this** → Polygon limiter → key rotation → bk_orb → Bridge cosmetics → offhost-backup check | **Built + fully validated 9/10 AM** (`scripts/signal_center_archive_rotate.py`, verify-then-delete per batch, archive triggers, auto-backup, offhost_backup.sh + weekly cron wired) against an isolated copy of the real DB — see `data/reports/relay/relay_2026-09-10_signal-center-retention-build.md`. **Not yet run against production** — holding for the given after-close queue order and Phase 1.1 to go first. 30-day window confirmed safe: only unbounded consumer found (`super_backtest_v2.py::analyze_signal_center`) is dead code, called only from retired `_archive/` scripts. |
...
| ThetaData integration | decision | parked — revisit after fills land | **Filed 2026-09-12, deliberately not built.** Free-tier account created, `ThetaTerminalv3.jar` downloaded to bigmac's home dir — but their API runs through a local Java terminal process, and bigmac has no JRE installed. **Reasoning for parking, not just "not done yet":** ThetaData answers "what were past options trades worth" (historical pricing) — real, but lower-value right now than the real-fills work shipped 2026-09-12 (`relay_2026-09-12_options_real_fills_shipped.md`), which makes *future* options rows verifiable by construction instead of needing a historical-pricing lookup after the fact at all. Revisit once the fills work has been live long enough to show which strategy (if any) actually has a real, verified trading record worth spending a JRE install and an integration on pricing further — per today's restatement findings, most of this system's historical options "profit" turned out to be unverifiable synthetic-formula pricing or bulk-reconciliation artifacts (`relay_2026-09-12_options_restatement_gap_closed.md`), so installing a historical-pricing tool before knowing which strategy earns that investment would be solving the wrong problem first. **Do not install a JVM on bigmac for this without re-reading this reasoning** — if fills land and a real strategy emerges, that's the trigger to revisit, not a calendar date. |
| Options premium restatement | Scotty | DONE 2026-09-11 | **Shipped** (dry-dock B6) — `options_trades` gained 4 additive columns + `options_trades_restated` view (15/120 pre-fix rows reconstructed from real Alpaca bars, 93 marked unrecoverable with a verified per-row reason, mostly a genuine TQQQ/SOXL/UPRO Alpaca liquidity gap). Season 1's 27 already-flagged `trades` rows reconfirmed unrecoverable for a stronger reason (no strike/expiry ever recorded — not a data-durability gap, a contract-identity gap). Season 1/4 do NOT become PSR/DSR-scoreable (insufficient volume); Season 2 DOES (6 players, none graduate). New finding: `options_trades` has a live units inconsistency (two code paths write entry_credit_debit in different scales, mult=100 vs mult=1) — flagged below, not fixed. Full detail: `relay_2026-09-11_B6_options_premium_restatement.md`. |
| Options_trades units inconsistency (mult=100 vs mult=1) | Scotty | unslotted | **Found 2026-09-11** building the B6 restatement script: CSP/wheel-strategy rows write `entry_credit_debit` as `entry_price*qty*100` (dollar-scale) while the older `strategy:bull_spread_v1` equity-spread rows write it as raw `entry_price` difference (no multiplier, doesn't even scale by `qty`) — two different code paths, same column, different units. Detected per-row for the restatement (whichever convention reproduces the row's own recorded value), not fixed at the source — touches the live write path for a table `options-sosnoff`/Troi still actively uses, out of scope for a read-mostly restatement pass. DECISION NEEDED: normalize to one convention going forward, or document both as permanently valid and gate every consumer on structure/source instead. |
| `calibration_map.py`'s trade_fire→trade_id join is too narrow to ever be useful | Scotty | unslotted | **Found 2026-09-11 (dry-dock B9), reconfirmed 2026-09-11 (correction pass).** `load_calibration_data()` requires `decision_audit.event_type='trade_fire'` with `trade_id` populated JOINed to a closed trade with non-NULL `realized_pnl` — fleet-wide, no player filter. Verified: 193 `trade_fire` events join cleanly to a real trade fleet-wide, only 3 have settled (non-NULL pnl) — the other 190 are still open. **This part of the finding stands.** (The "169 vs 168, reconciled" framing this row originally carried does NOT stand — that 168-trade comparison was fleet-wide, not McCoy-specific, and coincidentally close to the withdrawn 169 claim; see `relay_2026-09-11_calibration_finding_correction.md` for the full correction — the true McCoy-only figure in that bucket is 42 trades, 71.4% hit rate.) **The real finding, unaffected by the correction: the trade_fire→trade_id linkage `calibration_map` depends on is populated for almost no settled trades fleet-wide (3/~500+ = well under 1%)** — a design choice that makes the map's own stated methodology nearly unusable regardless of how much more data accumulates, unless the linkage itself gets backfilled/populated more broadly. Worth a decision: relax `calibration_map` to use a properly player-scoped direct `trades`-table query instead, or invest in backfilling `trade_id` on `trade_fire` events. **Precision correction, 2026-09-11 (Phase 1.3 enablement-bar investigation):** "backfilling `trade_id`" is not actually possible or needed — verified directly, 198/198 fleet-wide `trade_fire` events already have a populated `trade_id`, and all 198 resolve cleanly to a real `trades` row (0 dangling). The real constraint is settlement lag, not a missing-linkage bug: 195 of those 198 are matched to a trade that simply hasn't closed yet (no realized P&L to score against), only 3 have settled. The "relax `calibration_map` to a direct player-scoped query" option is the only one of the two that actually addresses this — there is no `trade_id` to backfill. |
| Dataset exporter (prompt_text/decision/invalidation/1d-5d return/regime/model) | Scotty | UNBLOCKED 2026-09-11 | Was held on the options premium restatement above landing first — **that landed** (B6, commit pending). RULE #1: writes new files only, never touches source tables. Walk-forward split by date. Next in the dry-dock B-item sequence (B7). |
| RESOLVED: engine/setup_similarity_signal.py still pointed at bigmac local Ollama | Scotty | — | **CORRECTED 2026-09-10 — the earlier "com.ollama.serve down" framing of this row was wrong, per the Captain.** `com.ollama.serve` was **deliberately** booted out + disabled on bigmac 2026-09-09 when all inference and embeddings moved to olliemax — not an outage, and it must NOT be restarted (would stand up a second Ollama server competing with the one actually serving). The real bug: `RECALL_OLLAMA_URL` was never actually added to `.env` in that migration (confirmed: not in `.env`, no launchd plist, no shell profile), so `engine/setup_similarity_signal.py`'s `OLLAMA_EMBED_URL = os.environ.get("RECALL_OLLAMA_URL", "http://127.0.0.1:11434/api/embed")` silently fell through to its stale local default. **Fixed**: now derives from `config.OLLAMA_URL` (the one already-migrated source of truth) instead of a second independently-configured fallback pair. **RESOLVED 2026-09-10, root cause found and fixed by the Captain**: initial live measurement showed 10-17s per call, wildly inconsistent — root cause was olliemax's `OLLAMA_MAX_LOADED_MODELS=2` with 3 models wanting residency (bge-m3 kept losing the slot, likely partly to `dr_crusher_check()`'s stray 5-min healthchecks against a halted seat's model, see the dayblade-0dte/0xroyce finding below). Raised to 3 on olliemax. **Re-measured post-fix: 0.171s cold, 0.035-0.045s warm — recall-in-prompt is viable as originally specced at these numbers**, no architecture change required (a precomputed-embeddings table remains the better long-term design but is no longer mandatory for latency reasons alone). This was a symptom of a much larger repo-wide pattern — see the next item. |
| Repo-wide hardcoded-Ollama-host sweep | Scotty | unslotted | **Found 2026-09-10** while fixing the row above — "third partial migration this week caused by a hardcoded local address," per the Captain. Full grep for `127.0.0.1:11434`/`localhost:11434` AND the same disease with pre-migration IPs (`192.168.1.166`, old Ollie Box; `192.168.1.168`, old Ollie Max) found ~30 live-code hits across `main.py`, `engine/war_room.py`, `engine/crew_scanner.py`, `engine/paper_trader.py`, `engine/ic_squadron.py`, `engine/wb_advisory_team.py`, `engine/portfolio_optimizer.py`, `engine/rebalancer.py`, `engine/fleet_status.py`, `agents/sarek.py`/`surak.py`/`janeway.py`, `dashboard/app.py`, `watchdog.py`, `engine/reveille.py`, `engine/phaser_lock.py`, `engine/archer/brain.py`, plus a dozen offline/manual scripts. Full inventory + tiering (fleet-critical needs a coordinated fix + restart vs. offline tools safe to fix standalone) relayed to the Captain 2026-09-10 for a sequencing decision. **Decision: Tier 3 (offline/manual tools, no restart needed) done 2026-09-10 evening — see commit for the file list. Tier 2 DONE 2026-09-11 (dry-dock B5, commit a1b8e64) — all 18 fleet-critical files fixed (the "16" above undercounted sarek/surak/janeway as one entry; live re-sweep found 18 real files, all 18 had live hits, all 18 fixed): env-read, fail loud, no localhost/stale-IP fallback. Two real live bugs found in the process, not just hygiene: `dashboard/app.py`'s `_ollama_chat` was posting to a raw hardcoded `localhost:11434` literal with no env read at all (the bigmac-local instance its own docstring says was retired 2026-06-24 — silently dead since then), and `watchdog.py`'s fallback would have silently re-introduced the exact "checking a host nothing calls" bug the 2026-06-14 HM-OLLAMA-WATCH-RETARGET fix already retired once. Full detail: commit message on a1b8e64.** |
| dr_crusher_check() healthchecks a halted seat's model | Scotty | unslotted | **Found 2026-09-10** investigating a live-Ollama-log finding (0xroyce/plutus:latest resident on olliemax with real calls tonight, interleaved with plutus-v1). Traced to `main.py`'s `dr_crusher_check()` (5-min healthcheck cadence) pinging `0xroyce/plutus` with a trivial "ok"/3-token completion to check "is the scan model responsive" — the model `dayblade-0dte` (T'Pol, 0DTE options, hardcoded in `main.py:161`'s `initialize_dayblade()`, bypassing `config.AI_PLAYERS`) is configured to use. **`dayblade-0dte` has been `halt_mode='full'` since 2026-07-13** (systematic options mispricing, reasoning/action inversion, fractional qty bugs — explicit halt_reason on file) — zero decisions, zero trades today. The healthcheck was never retargeted when the seat it monitors was paused, so it's been keeping a model no live seat uses resident for ~2 months, plausibly contributing to the GPU-residency contention that was also starving bge-m3 (see the row above) before `OLLAMA_MAX_LOADED_MODELS` was raised. Already flagged once, in a 2026-09-08 relay doc, and not actioned then either. **RETIRED 2026-09-11 (dry-dock C12)**, per the directive's explicit "retire it": the `0xroyce/plutus` ping (Check 2) removed from `dr_crusher_check()` entirely, not retargeted — no live seat needs a stray infra healthcheck pinning a specific model resident, and the function's other checks (DB liveness, recent-scan-activity) already cover real health without needing a model-specific probe. Also fixed a stale comment ("every 5 min") that didn't match the actual `schedule.every(15).minutes` cadence. Third time this was found; first time actioned. |
| trades.confidence=0.0 placeholder contamination | Scotty | unslotted | **Found 2026-09-10 building the calibration map (item B.2).** Of 474 clean closed trades with a non-null `confidence`, **305 (64%) are exactly `0.0`** — almost certainly a missing/default value from players or an era that never populated real confidence, not a genuine "stated zero confidence" prediction. The remaining 169 real values cluster almost entirely at 0.9/0.99/1.0, with essentially nothing in 0.5-0.8. Anyone querying `trades.confidence` for an average/distribution without excluding `0.0` will get a badly wrong number. Not root-caused tonight (which player_ids/era account for the 305) — flagging so a future query doesn't get fooled by it. |
| DECISION — revive SCAN_PAIRS/Gate 7 for ollama-plutus, or retire it | Steve | unslotted | **Found 2026-09-11 tracing McCoy's real decision path (dry-dock A4).** `ollama-plutus`'s alpha-squad slot (`SCAN_PAIRS`, feeds `_scan_single_agent()`/Gate 7 — composite_alpha/confidence checks — every 2 min, reaches real `paper_trader.buy()`) has been commented out since 2026-09-09, originally "for TODAY ONLY" over a VRAM collision with `qwen3:30b-a3b` that's since resolved (verified live 2026-09-11: the 30B isn't even loaded on olliemax right now). Comment corrected in `engine/crew_specialization.py` to stop claiming a temporary pause it wasn't. **The real number: 272 decisions logged to `crew_decisions` in the 30 days before the pause, 0 executed** — this path was already producing nothing while active. Reviving it should be a deliberate call weighed against that number, not a default restore now that the original blocking reason is gone. McCoy's real trading happens through a separate mechanism entirely (Arena/`decision_audit`) — this path is not load-bearing for that. |
...
## 🟢 HOLD LIFTED — season rotation no longer orphans positions (set + lifted 2026-09-13)

The 2026-09-13 hold (no rotation, auto or manual) is lifted by the commit that added this
section. **What now makes rotation safe** (`engine/season_manager.py`,
HM-ROTATION-POSITIONS-ARCHIVE-2026-09-13):
- Rows are never cleared for humans, passive broker mirrors, independent players
  (`neo-matrix` — its rows are owned by `shared/matrix_bridge`; the old hardcoded
  exclusion is kept explicitly), or holders not in `ai_players`.
- Any other row whose symbol is open at the broker **blocks the rotation before any write**.
  Broker presence is read live from Alpaca, else from an `alpaca-mirror` snapshot ≤6h old;
  if neither can be read the rotation **fails closed** (aborts, alerts, writes nothing).
- Every other row goes through the **house archive-then-clear pattern**
  (`scripts/signal_center_archive_rotate.py::_rotate_table`): INSERT into append-only
  `positions_season_archive` (every positions column verbatim + archive_season/name/at/reason),
  read back, verify row-for-row — count AND every field — then DELETE that batch; any failure
  rolls back the whole rotation. Same `trg_rule1_no_delete_*` triggers (plus a no-update
  trigger), ensured at apply time; a one-time integrity-checked snapshot before the first-ever
  archive run, recorded in `archive_metadata` so it can neither repeat nor be skipped.
- **Dry-run by default:** `rotate_season()`/`start_season()` return a plan and write nothing
  unless `apply=True` (dashboard `/api/seasons/start` needs `"apply": true`).
- The ending season's `season_config.end_date` is filled; Season 7 backfilled from
  `season_history.ended_at`.

**Still true:** `SEASON_AUTOROTATE_ENABLED` stays **off**; seasons are manual and no rotation
runs until the Admiral says go.

## Ledger — 2026-09-13 (record only, no code)
- **Auto-rotate sentinel ordering:** the once-per-Sunday marker is held in memory with the
  DB "season already started today" check behind it. The DB check should be primary and
  memory only an optimization — same restart-wipe shape as the 2026-08-28 neo-matrix
  duplicate-fire.
- **Bridge honesty:** Riker's 2026-09-11 "stops in place on AG, AVB, BLK" restated Spock's
  RECOMMENDED stop levels (`cto_briefings` portfolio advisory) as existing orders. No
  matching broker stops exist (Alpaca: no stop/stop_limit orders in the last 50; no
  `stop_loss_order_id` on any trade). Same family as the 2026-09-12 Bridge audit items.
