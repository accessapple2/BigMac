...
### Open INFRA sign-offs (Admiral gate) — 3 of 4 DONE
1. ~~DNS apex record + redirect~~ — **DONE**, verified live. Query-string-preservation sub-fix: **token provided by Admiral, but I could not use it directly** — writing it into a Bash `export` command was blocked by this session's own credential-leakage classifier (persists in shell history/tool logs). Gave the Admiral the exact ready-to-paste `curl` sequence (GET rulesets → GET rule → PUT with `preserve_query_string` fixed) using `$CF_TOKEN` as a literal placeholder for them to run themselves. **Not yet applied** — awaiting the Admiral running those commands; I verify afterward with `curl -I "http://ollietrades.com/foo?bar=1"` (no token needed for that check).
...
3. **manifest.json CF Access bypass** — **not applied by me, by design.** Admiral is doing this one directly in the dashboard; my role was confirming the exact path, which is `/static/manifest.json` (not bare `/manifest.json`). I also don't have Zero Trust Access-policy write access via my current token (scoped to Zone Rulesets only), so I couldn't apply it even if asked. Draft steps at `docs/DRAFT_manifest_cf_access_bypass.md` if useful as a reference while working in the dashboard.
4. ~~status.ollietrades.com~~ — **DONE**, live and verified (`f2d929c`, see above).

### INCIDENT — erroneous DNS record on a different zone, needs Admiral cleanup
While attempting the status-page DNS route myself (before the Admiral did it via dashboard instead), `cloudflared tunnel route dns trademinds status.ollietrades.com` did not create `status.ollietrades.com` — it created **`status.ollietrades.com.accessapple.com`** (confirmed via `dig`: real record, resolves to real Cloudflare proxy IPs). The cloudflared CLI's login cert on this box appears scoped to a different zone/account (`accessapple.com`, matching the GitHub org `accessapple2` this repo pushes to) than `ollietrades.com`'s zone ("Bonnie's Account") — passing a hostname that doesn't end in `.accessapple.com` caused it to nest rather than error. No delete subcommand exists in the CLI, and my API token is scoped to `ollietrades.com` Rulesets only, not `accessapple.com` DNS — **I cannot remove this myself. Needs Admiral cleanup**, not yet confirmed done as of this writing. Side lesson: this also explains why the existing 4 tunnel hostnames must have been set up via the dashboard originally, not this CLI — the CLI can't write to the `ollietrades.com` zone at all from this login state.
