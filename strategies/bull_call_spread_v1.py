...
def _get_spot(ticker: str) -> Optional[float]:
    ...
    try:
        ...
        if spot is not None:
            return spot
        print(f"[bull_call_spread_v1] WARN: Polygon spot=None for {ticker}, falling back to Alpaca")
    except Exception as e:
        print(f"[bull_call_spread_v1] WARN: Polygon spot exception for {ticker}: {e}, falling back to Alpaca")
