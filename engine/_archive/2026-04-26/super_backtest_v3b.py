"""
engine/super_backtest_v3b.py — Warp Core Reactor v3b — Lean Fleet Shakedown Cruise (Fixed)

90-day backtest (Oct 9 2025 → Apr 9 2026) of the 12-agent lean fleet.

Bug fixes over v3:
  • Fix 1: Per-agent P&L explosion — cap each trade pnl_pct at ±100% before compounding
  • Fix 2: Seven of Nine (0 trades) — add Bollinger Band + SMA golden cross signals
  • Fix 3: Trade counter = 0 in comparison table — key name mismatch fixed (total_bt_trades)
  • Fix 4: Shelve T'Pol (dayblade-0dte) — 0DTE with execution delay = guaranteed loss
...
Enhancements over v2:
  ...
  • Four-way comparison table (Actual Fleet / v1 / v2 / v3b)
  • Alpha attribution by signal component (dark_pool, earnings, etc.)
  • T'Pol 0DTE loop (SHELVED: 0DTE with execution delay = guaranteed loss)
...
LEAN_FLEET: dict[str, dict] = {
    "deepseek-7b-grok4":           {"name": "Spock",   "model": "deepseek-r1:14b", "tiers": [1],
                         "specialization": "mean_reversion"},
    "qwen3-8b-flash": {"name": "Worf",    "model": "qwen3:14b",       "tiers": [5, 6, 9],
    ...
    "dayblade-0dte":    {"name": "TPol",    "model": "options-s2",      "tiers": [8],
                         "active": False,  # SHELVED: 0DTE with execution delay = guaranteed loss
                         "specialization": "0dte"},
...
def _trade_metrics_v3b(trades: list[dict]) -> dict:
    """Fixed version: drawdown capped at -100%, proper equity curve.
    Fix 1: cap each individual trade pnl_pct at ±100% to prevent options P&L explosion."""
    ...
    pcts   = [max(-100.0, min(100.0, t["pnl_pct"])) for t in trades]  # cap ±100%: prevents options P&L explosion from cheap-stock CSP/spread formulas
...
def _init_v3b_tables() -> None:
    conn = _bt_conn()
    conn.executescript("""
    CREATE TABLE IF NOT EXISTS backtest_agent_results_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_master_results_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_equity_curve_v3b (
        run_date TEXT, trade_date TEXT, equity REAL,
        daily_pnl REAL, regime TEXT,
        PRIMARY KEY (run_date, trade_date)
    );

    CREATE TABLE IF NOT EXISTS backtest_monthly_breakdown_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_regime_results_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_alpha_attribution_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_options_results_v3b (
    ...
    CREATE TABLE IF NOT EXISTS backtest_spread_results_v3b (
        run_date TEXT, strategy TEXT, ticker TEXT,
        pnl REAL, pnl_pct REAL, exit_type TEXT,
    ...
    conn.commit()
    conn.close()
    logger.info("v3b tables initialized")
...
def _run_event_loop_v3b(td: dict, days: list, vix_map: dict,
    ...
    logger.info(f"Event loop v3b: {total} trades across {len(agent_trades)} agents")
...
def _run_options_loop_v3b(td: dict, days: list, vix_map: dict,
    ...
    logger.info(f"Options loop v3b: {len(options_trades)} opts, {len(spread_trades)} spreads, "
                f"{skipped} skipped by alpha")
    return options_trades, spread_trades


# ═══════════════════════════════════════════════════════════════════════════════
# 0DTE loop — T'Pol (SPY + QQQ, regime-gated) — SHELVED
# ═══════════════════════════════════════════════════════════════════════════════

def _run_0dte_loop_v3b(td: dict, days: list, vix_map: dict) -> list:
    ...
    logger.info(f"0DTE loop v3b: {len(trades)} trades")
    return trades
...
def _build_equity_curve_v3b(event_trades: dict, opt_trades: list, spread_trades: list,
...
def _compute_alpha_attribution_v3b(run_date: str, all_trades: list) -> dict:
...
def _save_agent_results_v3b(run_date: str, agent_metrics: dict) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    for aid, m in agent_metrics.items():
        spec = LEAN_FLEET.get(aid, {})
        conn.execute("""
            INSERT OR REPLACE INTO backtest_agent_results_v3b
...
def _save_master_results_v3b(run_date: str, event_trades: dict, opt_trades: list,
...
def _save_equity_curve_v3b(run_date: str, curve: list[dict]) -> None:
    conn = _bt_conn()
    for row in curve:
        conn.execute("""
            INSERT OR REPLACE INTO backtest_equity_curve_v3b
...
def _save_options_results_v3b(run_date: str, opt_trades: list) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    for t in opt_trades:
        if "pnl_pct" not in t:
            continue
        conn.execute("""
            INSERT INTO backtest_options_results_v3b
...
def _save_spread_results_v3b(run_date: str, spread_trades: list) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    for t in spread_trades:
        if "pnl_pct" not in t:
            continue
        conn.execute("""
            INSERT INTO backtest_spread_results_v3b
...
def _save_0dte_results_v3b(run_date: str, dte0_trades: list) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    for t in dte0_trades:
        ...
        conn.execute("""
            INSERT INTO backtest_0dte_results_v3b
...
def _save_monthly_v3b(run_date: str, monthly_all: dict, monthly_by_agent: dict) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    ...
    for month, trades in monthly_by_agent.items():
        for aid, atrades in trades.items():
            ...
            conn.execute("""
                INSERT OR REPLACE INTO backtest_monthly_breakdown_v3b
...
def _save_regime_results_v3b(run_date: str, regime_metrics: dict) -> None:
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()

    for (regime, strat), m in regime_metrics.items():
        conn.execute("""
            INSERT OR REPLACE INTO backtest_regime_results_v3b
...
def _save_alpha_attribution_v3b(run_date: str, alpha_attr: dict) -> None:
    conn = _bt_conn()
    for sig, data in alpha_attr.items():
        conn.execute("""
            INSERT OR REPLACE INTO backtest_alpha_attribution_v3b
...
def _build_comparison_v3b(run_date: str, fleet_metrics: dict, summary: dict) -> dict:
    """Build four-way comparison: Actual Fleet / v1 / v2 / v3b."""
    conn = _bt_conn()
    now  = datetime.utcnow().isoformat()
    ...
    try:
        v3_sharpe = _safe_float(conn.execute(
            f"SELECT AVG(realistic_sharpe) FROM backtest_master_results_v3b "
            f"WHERE run_date=? AND num_trades > 0", (run_date,)
        ).fetchone())
        v3_dd = _safe_float(conn.execute(
            f"SELECT MIN(max_drawdown) FROM backtest_master_results_v3b "
            f"WHERE run_date=? AND num_trades > 0", (run_date,)
        ).fetchone())
        v3_wr = _safe_float(conn.execute(
            f"SELECT AVG(win_rate) FROM backtest_master_results_v3b "
            f"WHERE run_date=? AND num_trades > 0", (run_date,)
        ).fetchone())
    except Exception:
        v3_sharpe = v3_dd = v3_wr = 0.0
    v3_trades = summary.get("total_bt_trades", 0)  # FIXED: matches summary.get("total_bt_trades")
    ...
    KNOWN = {
        ...
        "v3":            {"return": -36.99, "sharpe": -0.034, "wr": 48.8,  "trades": 153,   "agents": 11},  # v3 buggy baseline
    }
    ...
    for metric, af, af_l, v1v, v1_l, v2v, v2_l, v3v, v3_l in rows:
        conn.execute("""
            INSERT OR REPLACE INTO comparison_table_v3b
...
def _print_report_v3b(summary: dict, comparison: dict) -> None:
    ...
    print(f"\n{bar}")
    print(f"  WARP CORE REACTOR v3b — Lean Fleet Shakedown Cruise (Fixed)")
    print(f"  Super Backtest v3b  |  90-day window (Oct 9 2025 → Apr 9 2026)")
    ...
    try:
        ...
        strat_rows = conn.execute("""
            ...
            FROM backtest_master_results_v3b
    ...
    try:
        ...
        months = sorted({r["month"] for r in conn.execute(
            "SELECT DISTINCT month FROM backtest_monthly_breakdown_v3b WHERE run_date=?",
            (summary["run_date"],)
        ).fetchall()})
        if months:
            col_w = 12
            ...
            for aid in agent_ids:
                ...
                for m in months:
                    r = conn.execute("""
                        SELECT total_return FROM backtest_monthly_breakdown_v3b
    ...
    print(f"  Fleet ({fleet_agents} agents) | v1 (29 agents, no alpha gate) | "
          f"v2 (12 agents, alpha gate) | v3b (12 agents, agent routing + bug fixes)")
    ...
    print(f"  v3b:          uses fixed formula throughout")
...
def run_super_backtest_v3b(days: int = BACKTEST_DAYS) -> dict:
    """
    Full Super Backtest v3b pipeline — Lean Fleet Shakedown Cruise (Fixed).
    Saves all results to data/backtest.db with _v3b tables.
    ...
    logger.info("═" * 60)
    logger.info("  Warp Core Reactor v3b — Lean Fleet Shakedown Cruise (Fixed)")
    ...
    logger.info(f"[STEP 5B] Running v3b backtest on {len(td)-1} symbols, "
                f"{len(trading_days)} trading days...")

    event_trades, agent_trades = _run_event_loop_v3b(
        td, trading_days, vix_map, alpha_scores, conf_scores
    )
    opt_trades, spread_trades = _run_options_loop_v3b(
        td, trading_days, vix_map, alpha_scores
    )
    dte0_trades = []  # T'Pol shelved: 0DTE with execution delay = guaranteed loss (dayblade-0dte set inactive)

    # Tag options/spreads with agent tracking
    for t in opt_trades:
        strat = t.get("strategy", "")
        aid   = t.get("agent_id", "qwen3-8b-flash" if ("bear" in strat or "put" in strat) else "ollama-llama")
        t["agent_id"] = aid
    ...
    curve = _build_equity_curve_v3b(event_trades, opt_trades, spread_trades,
    ...
    regime_metrics = {k: _trade_metrics_v3b(v) for k, v in regime_perf.items() if len(v) >= 2}
    ...
    alpha_attr = _compute_alpha_attribution_v3b(run_date, all_trades_flat)

    # ── Save results ──────────────────────────────────────────────────────────
    logger.info("[STEP 6] Saving v3b results to DB...")
    _save_agent_results_v3b(run_date, agent_metrics)
    _save_master_results_v3b(run_date, event_trades, opt_trades, spread_trades, spy_return)
    _save_equity_curve_v3b(run_date, curve)
    _save_options_results_v3b(run_date, opt_trades)
    _save_spread_results_v3b(run_date, spread_trades)
    _save_0dte_results_v3b(run_date, dte0_trades)
    _save_monthly_v3b(run_date, monthly_all, monthly_by_agent)
    _save_regime_results_v3b(run_date, regime_metrics)
    _save_alpha_attribution_v3b(run_date, alpha_attr)

    # ── Four-way comparison ───────────────────────────────────────────────────
    equity_end = curve[-1]["equity"] if curve else STARTING_CASH
    comparison = _build_comparison_v3b(run_date, fleet_metrics, {
        "equity_end":      equity_end,
        "total_bt_trades": len(all_trades_flat),   # FIXED: matches summary.get("total_bt_trades")
    ...
    summary = {
        ...
        "regime_metrics": {
            ...
            and _trade_metrics_v3b(regime_perf[(r, s)]) == regime_metrics.get((r, s), {})
        },
    ...
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        summary_path = DATA_DIR / "backtest_v3b_summary.json"
    ...
    _print_report_v3b(summary, comparison)
    return summary
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [sv3b] %(levelname)s %(message)s",
    )
    parser = argparse.ArgumentParser(
        description="Super Backtest v3b — Warp Core Reactor / Lean Fleet Shakedown Cruise (Fixed)"
    )
    ...
    if args.fleet_only:
        ...
        result = run_super_backtest_v3b(args.days)
        ...
        print(f"Summary: data/backtest_v3b_summary.json")
