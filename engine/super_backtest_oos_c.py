"""
engine/super_backtest_oos_c.py — OOS Candidate C: 2022 Bear Regime Test

Regime-robustness check. SPY –24.3% in 2022 (rate-hike bear + Russia/Ukraine).
Tests whether CSP Sharpe 6.05 from Candidate A (bull 2024) was regime-dependent
or reflects genuine structural edge.

DIFFERENCES FROM super_backtest_oos.py (Candidate A):
  • Window: 2022-01-03 → 2022-10-14 (~198 trading days; full bear + rate shock)
  • Universe: Point-in-time S&P 500 as of Jan 2022 (83 post-2022 changes reversed)
    - 32 acquired/delisted tickers unavailable in yfinance (SIVB, FRC, TWTR, ATVI,
      CERN, etc.) — documented; notable that SIVB/FRC are catastrophic misses for
      CSP strategy (sell-put into failing bank = max loss). Slight anti-bear bias.
  • Alpha scores: neutral 0.5 (no look-ahead; consistent with Candidate A)
  • Gate thresholds: identical to Candidate A (CLAUDE.md spec)
    - sniper alpha 0.25, neo-matrix 1.75, chekov 5.0 (retired), qwen3/capitol 2.7
  • Results saved to _oos_c tables — neither v5 nor _oos tables touched
  • Per-strategy evaluation is the primary output — long-biased strategies
    (rsi_bounce, bull_momentum) are expected to underperform structurally.
    The key question is whether CSP produces positive Sharpe in a –24% SPY year.

Run:
    venv/bin/python3 -m engine.super_backtest_oos_c 2>&1 | tee data/oos_c_run.log
"""
...
WINDOW_START  = date(2022, 1, 3)    # OOS-C: 2022 bear regime start
WINDOW_END    = date(2022, 10, 14)  # ~198 trading days; covers full bear + rate shock
BACKTEST_DAYS = 1900                # must reach Jan 2022 from Apr 2026 (~4.3yr) + 200d warmup
...
def _init_oos_c_tables() -> None:
    """Create OOS-C result tables with _oos_c suffix. Never modifies v5 tables."""
    conn = _bt_conn()
    conn.executescript("""
    CREATE TABLE IF NOT EXISTS backtest_results_oos_c (
    ...
    CREATE TABLE IF NOT EXISTS backtest_trades_oos_c (
    ...
    CREATE TABLE IF NOT EXISTS backtest_decisions_oos_c (
    ...
    CREATE TABLE IF NOT EXISTS backtest_summary_oos_c (
...
def _build_pit_universe_2022() -> list[str]:
    """
    Reconstruct S&P 500 constituents as of 2022-01-03 using Wikipedia changes table.

    Method:
      1. Fetch current S&P 500 from Wikipedia (503 tickers)
      2. Parse changes table; reverse all changes dated >= 2022-01-03
         (remove tickers ADDED after Jan 2022, re-add tickers REMOVED after Jan 2022)
      3. Validate via yfinance: confirm 2022 price data exists, price >= $3
      4. Rank by 2022 avg daily volume (Jan-Oct) → return top 200

    Limitations:
      - 32 delisted/acquired tickers (SIVB, FRC, TWTR, ATVI, CERN, etc.) are unavailable
        in yfinance post-delisting and cannot be included despite being active in 2022.
    ...
    cache_path = DATA_DIR / "pit_universe_2022.json"
...
def _run_sniper_v5(td: dict, trading_days: list, vix_map: dict,
    ...
    for aid, am in agent_metrics.items():
        ...
        conn.execute("""
            INSERT OR REPLACE INTO backtest_results_oos_c
    ...
    for (strat, sym), trades in by_strat_sym.items():
        ...
        conn.execute("""
            INSERT OR REPLACE INTO backtest_trades_oos_c
    ...
    for row in curve:
        conn.execute("""
            INSERT OR REPLACE INTO backtest_equity_oos_c
    ...
    for month, agents_dict in monthly_by_agent.items():
        for aid, atrades in agents_dict.items():
            ...
            conn.execute("""
                INSERT OR REPLACE INTO backtest_monthly_oos_c
    ...
    for d in all_ollie_decisions:
        ...
        conn.execute("""
            INSERT INTO backtest_decisions_oos_c
    ...
    conn.execute("""
        INSERT OR REPLACE INTO backtest_summary_oos_c
    ...
    summary_path = DATA_DIR / "backtest_oos_c_summary.json"
...
def run_oos_backtest_2022_c() -> dict:
    """
    OOS-C Validation: held-out 2022 bear regime backtest.
    Runs the SAME strategy/gate logic as v5 on data it has never seen.
    NEVER modifies trader.db, arena.db, or any v5 tables.
    Results written to *_oos_c tables only.
    """
    t0       = time.time()
    ...
    logger.info("═" * 70)
    logger.info("  OOS-C Validation: Held-Out 2022 Bear Regime Backtest (Candidate C)")
    ...
    logger.info("[STEP 2] Building point-in-time S&P 500 universe as of 2022-01-03...")
    pit_symbols = _build_pit_universe_2022()
    ...
    logger.info(f"OOS-C Backtest 2022 complete in {elapsed}s")
    logger.info(
        f"Results in backtest.db tables: backtest_results_oos_c, backtest_trades_oos_c, "
        f"backtest_summary_oos_c, backtest_equity_oos_c, backtest_monthly_oos_c, backtest_decisions_oos_c"
    )

    return {"status": "ok", "run_date": run_date, "sniper_oos_c": bC, "elapsed_s": elapsed}


if __name__ == "__main__":
    run_oos_backtest_2022_c()
